...
indL3 = en_bs>=enL3start & en_bs<=envalley;
indL2 = en_bs>=envalley & en_bs<=enL2end;
...
[~,ind1] = min(abs(en_bs-enL3start));
[~,ind2] = min(abs(en_bs-en2));
[~,ind3] = min(abs(en_bs-enL2end));

initvalue = 0;
midvalue = mean(sig_bs(ind2-2:ind2+2));
endvalue = mean(sig_bs(ind3-1:ind3+1));
areaL3 = sum(sig_bs(ind1:ind2))-(0+midvalue)*(ind2-ind1+1)/2;
areaL2 = sum(sig_bs(ind2:ind3))-(midvalue+endvalue)*(ind3-ind2+1)/2;
L32(5) = areaL3/areaL2;
L3(5) = areaL3;
L2(5) = areaL2;

Fe3_normalize(i).area = L3; 
